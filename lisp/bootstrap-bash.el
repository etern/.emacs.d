...
(defun config-history ()
  ...
  (let* ((ignore-str (getenv "HISTIGNORE"))
    ...
    (unless (seq-some (lambda (str) (string= str "fg")) ignores)
      (setq ignores (cons "fg" ignores)))
    (unless (seq-some (lambda (str) (string= str "ll")) ignores)
      (setq ignores (cons "ll" ignores)))
    (when (> (length ignores) orig-num)
      (with-current-buffer (find-file "~/.bashrc")
	(goto-char (point-max))
	(unless (char-equal (char-before) ?\n) (insert "\n"))
	(insert "HISTIGNORE=\"" (mapconcat 'identity ignores ":") "\"")
	(save-buffer)
	(kill-buffer (current-buffer))))))
